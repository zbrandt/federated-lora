...
def _plot_task(task: str, method_runs: dict[str, list[Run]], colors: dict[str, str], output_path: Path) -> None:
    ...
    for method in sorted(method_runs):
        ...
        plt.plot(rounds, mean, marker="o", markersize=3, linewidth=2, label=label, color=color)
        if np.any(std > 0):
            plt.fill_between(rounds, mean - std, mean + std, alpha=0.15, color=color)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Plot accuracy against round from result data")
    parser.add_argument(
        "inputs",
        nargs="*",
        default=["results"],
    ...
    parser.add_argument("--output-dir", default="figures", help="directory for the figures")
