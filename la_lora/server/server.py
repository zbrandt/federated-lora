...
class Server:
	...
	def run(self) -> list[RoundMetrics]:
		...
		for round_index in range(1, self.config.rounds + 1):
			...
			# Client computation
			states, losses, uploaded = client_computation(selected_clients, self.model, self.global_state)
			...
			self.model.load_state_dict(self.global_state, strict=False)	# TODO: figure out what this does
