...
class Client:
	...
	def local_update(self, model: PreTrainedModel, adapter_state: dict[str, torch.Tensor]) -> ClientResult:
		...
		"""
		model.load_state_dict(adapter_state, strict=False)	# TODO: figure out what this is
		...
		for step in range(1, self.config.local_steps + 1):
			batch = next(batches)
			batch = {key: value.to(self.device) for key, value in batch.items()}	# move batch items to the correct device
			...
			optimizer.zero_grad(set_to_none=True)	# reset the gradients of all optimized Tensors
			if opt_head:
				opt_head.zero_grad(set_to_none=True)
			loss = model(**batch).loss 				# access scalar loss tensor calculated with forward pass
